...
mod tests {
    ...
    fn single_value_none() {
        assert_eq!(parse_str("None"), Ok(Some(Value::None)));
        assert_eq!(parse_str("none"), Ok(Some(Value::None)));
        assert_eq!(parse_str("NONE"), Ok(Some(Value::None)));
        assert_eq!(parse_str("Null"), Ok(Some(Value::None)));
        assert_eq!(parse_str("null"), Ok(Some(Value::None)));
        assert_eq!(parse_str("NULL"), Ok(Some(Value::None)));
    }
    ...
    #[test]
    fn single_value_str() {
        assert_eq!(parse_str("\"hello\""), Ok(Some(Value::Str("hello".into()))));
    }

    #[test]
    fn single_value_num() {
        assert_eq!(parse_str("3.14"), Ok(Some(Value::Num("3.14".into()))));
        assert_eq!(parse_str("0"), Ok(Some(Value::Num("0".into()))));
        assert_eq!(parse_str("-99999"), Ok(Some(Value::Num("-99999".into()))));
    }

    #[test]
    fn single_value_bool() {
        assert_eq!(parse_str("true"), Ok(Some(Value::Bool(true))));
        assert_eq!(parse_str("false"), Ok(Some(Value::Bool(false))));
    }

    #[test]
    fn single_value_obj() {
        assert_eq!(parse_str("{}"), Ok(Some(Value::Obj(MapT::new()))));
        let a = Value::Obj(HashMap::from([(
            String::from("pi"),
            Value::Num(String::from("3.14")),
        )]));
        assert_eq!(parse_str("{pi: 3.14}"), Ok(Some(a)));
    ...
    fn single_value_list() {
        assert_eq!(parse_str("[]"), Ok(Some(Value::List(Vec::new()))));
        assert_eq!(
            parse_str("[2.71]"),
            Ok(Some(Value::List(vec![Value::Num(String::from("2.71"))])))
        );
        assert_eq!(
            parse_str("[\n\nfalse\t,]"),
            Ok(Some(Value::List(vec![Value::Bool(false)])))
        );
    }
